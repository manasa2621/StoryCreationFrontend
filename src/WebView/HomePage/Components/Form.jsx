  ...
  const handleSubmit = async (e, formData) => {
    ...
    if (props.isSignUp) {
      const response = await axios.post(
        "https://storycreationbackend.onrender.com/register",
        formData,
        {
          headers: {
    ...
    if (props.isLogIn) {
      const response = await axios.post(
        "https://storycreationbackend.onrender.com/login",
        formData,
        {
          headers: {
            "content-type": "application/x-www-form-urlencoded",
...
function signUp(formData, setFormData, handleSubmit, response) {
  return (
    <>
      <form
        action="https://storycreationbackend.onrender.com/register"
        method="POST"
        onChange={(e) =>
      ...
      >
        <p id="register">Register to SwipTory</p>
        <div >
          <label htmlFor="name"style={{ marginRight: "30px" }} >
            Name
          </label>
          <input
            type="text"
...
function logIn(formData, setFormData, handleSubmit, response) {
  return (
    <>
      <form
        action="https://storycreationbackend.onrender.com/login"
        method="post"
        onChange={(e) =>
          setFormData({ ...formData, [e.target.name]: e.target.value })
        }
